...
namespace Neurotrauma.ClientSource
{
    internal class ButtonsHUI
    {
        ...
        private static void ApplyTreatmentLocally(Character User, Character Target, int SelectedLimbIndex, Identifier ItemIdentifier)
        {
            ...
            NeurotraumaInit.NTItemsLoader.Get(ItemIdentifier.Value)?.Invoke(new NTItems.ItemUpdateFunctionInfos(null, 
                NTHuman.getNTHumanFromCharacter(User), 
                NTHuman.getNTHumanFromCharacter(Target), 
                TargetLimb));
        }
